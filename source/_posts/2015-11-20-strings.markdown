...
Python 2 中字符字面量（即 str 类型）表示的是`byte string`，也就是说`foo = "abc"`这一赋值语句表示的是把`a、b、c`所对应的 ASCII 码赋值给变量`foo`，也就是说在 Python 2 中，str 类型直接保存的是字符的 ASCII 码。在 Python 2如果想表示 Unicode 字符，需要在字符前面加`u`，像`foo = u"abc"`，此时保存的就不是字符的 ASCII 码了。
